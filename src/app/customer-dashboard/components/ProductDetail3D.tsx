...
import { RotateCcw, Maximize2, Smartphone, Info, ChevronRight, Camera } from 'lucide-react';
...
export default function ProductDetail3D() {
  ...
  function handleRotate() {
    setRotationIndex((value) => (value + 1) % rotationClasses.length);
  }

  function handleScaleUp() {
    setScaleIndex((value) => Math.min(scaleClasses.length - 1, value + 1));
  }
  ...
  const activeViewportClass =
    viewMode === 'photo'
      ? 'bg-slate-100 dark:bg-slate-900'
      : 'bg-gradient-to-br from-violet-200 via-purple-200 to-white dark:from-purple-950 dark:via-violet-900 dark:to-slate-950';

  const viewModes = [
    { key: 'photo' as ViewMode, label: 'Static Photo', desc: 'High-res inspection photo', emoji: '📷' },
        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          <div className="flex-1">
            <div className={`relative min-h-[320px] overflow-hidden rounded-2xl border border-border ${activeViewportClass} transition-colors`}>
              {viewMode === 'photo' && (
                <img
                  src="https://img.rocket.new/generatedImages/rocket_gen_img_1388d488a-1772061255574.png"
                  alt="Oak dining table product photo showing full table with natural wood grain finish"
              ...
              {viewMode === '3d' && (
                  ...
                  <div className={`relative mb-6 ${rotationClasses[rotationIndex]} ${scaleClasses[scaleIndex]} transition-transform duration-300`}>
                    <div className="flex h-32 w-48 items-center justify-center rounded-2xl border border-border bg-muted/80 shadow-lg backdrop-blur-md">
                      <div className="text-center">
                        ...
                        <p className="text-xs font-semibold text-foreground">Oak Dining Table</p>
                        <p className="text-[10px] text-muted-foreground">3D Preview</p>
                      </div>
                    </div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Interactive 3D model — WP-2847</p>

                  <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
                    <button
                      onClick={handleRotate}
                      className="flex items-center gap-1.5 rounded-xl border border-border bg-muted px-3 py-2 text-sm font-semibold text-foreground transition-all active:scale-95"
                    >
                      <RotateCcw size={14} /> Rotate
                    </button>
                    <button
                      onClick={handleScaleUp}
                      className="flex h-9 w-9 items-center justify-center rounded-xl border border-border bg-muted text-lg font-bold text-foreground transition-all active:scale-95"
                    >
                      +
                    </button>
                    <button
              ...
              {viewMode === 'ar' && arStep === 'intro' && (
                  ...
                  <p className="mb-2 max-w-xs text-sm text-slate-700 dark:text-white/80">
                    Point your camera at a flat surface to place your Oak Dining Table in your space.
                  </p>
                  <div className="mb-6 w-full max-w-xs space-y-2 text-left">
                    {[
                      'Point camera at a flat floor surface',
                      'Tap to place the furniture',
                      'Pinch to resize · Drag to reposition',
                    ].map((step, index) => (
                      <div key={step} className="flex items-center gap-2.5">
                        <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-slate-200 text-xs font-bold text-slate-700 dark:bg-white/20 dark:text-white">
                          {index + 1}
                        </div>
              ...
              {viewMode === 'ar' && arStep === 'camera' && (
                <div className="relative flex h-full min-h-[320px] items-center justify-center overflow-hidden">
                  ...
                  <div className={`relative z-10 text-center ${scaleClasses[scaleIndex]} transition-transform duration-300`}>
                    <div className="mx-auto flex h-28 w-40 items-center justify-center rounded-2xl border border-violet-300/60 bg-violet-400/25 backdrop-blur-sm dark:border-violet-200/40 dark:bg-violet-300/20">
                      <div className="text-center">
                        ...
                        <p className="mt-1 text-xs text-white/80">Oak Dining Table</p>
                      </div>
                    </div>
                  ...
                  <div className="absolute bottom-4 left-0 right-0 flex items-center justify-center gap-3 px-4">
                    <button
                      onClick={handleScaleDown}
                      className="flex h-10 w-10 items-center justify-center rounded-full border border-white/30 bg-white/15 text-lg font-bold text-white backdrop-blur-sm transition-all"
                    >
                      −
                    </button>
                    <button
                      onClick={handleRotate}
                      className="flex items-center gap-1.5 rounded-full bg-purple-600 px-4 py-2 text-sm font-semibold text-white transition-all hover:bg-purple-700"
                    >
                      <RotateCcw size={14} /> Rotate
                    </button>
                    <button
                  ...
                  <button
                    ...
                    className="absolute left-3 top-3 rounded-lg bg-black/50 px-3 py-1.5 text-xs font-semibold text-white"
                  >
                    ← Back
                  </button>
                  <p className="absolute right-3 top-3 text-xs text-white/60">AR Mode</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4 lg:w-64">
            <div>
              <h3 className="mb-1 text-base font-semibold text-foreground">Oak Dining Table</h3>
              <p className="text-xs text-muted-foreground">WP-2847 · ORD-4421 · Solid Oak, Natural Finish</p>
            </div>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (mode.key !== 'ar') setArStep('intro');
                    }}
                    className={`flex w-full items-center gap-3 rounded-xl border px-3 py-3 text-left text-sm transition-all duration-150 active:scale-95 ${
                      isActive
            ...
            <button
              ...
              className="flex w-full items-center justify-between rounded-xl border border-border bg-muted px-3 py-2.5 text-sm font-semibold text-foreground transition-all"
            >
            ...
            {showInfo && (
              <div className="space-y-2 rounded-xl border border-border bg-muted p-3 animate-fade-in">
                {specs.map((spec) => (
