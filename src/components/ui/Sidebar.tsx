...
export default function Sidebar({ role, currentPath, open, onClose, collapsed = false, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <>
      <aside
        className={`fixed top-16 bottom-0 left-0 z-40 ${collapsed ? 'w-20 p-4' : 'w-72 p-6'} overflow-y-auto border-r border-border bg-card transition-all duration-200 md:static md:translate-x-0 ${
        ...
        <div className="relative mb-6 h-16">
          {onToggleCollapse && (
            <button
              ...
              className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 md:inline-flex items-center justify-center rounded-full border border-border bg-background p-2 text-muted-foreground hover:text-foreground shadow-sm ${collapsed ? 'w-8 h-8' : 'w-9 h-9'}`}
          ...
          <button
            type="button"
            onClick={onClose}
            className="md:hidden absolute right-2 top-1/2 -translate-y-1/2 btn-ghost p-2"
